...
void Grid::generatePossibleDirections()
{
	...
	if (m_movement == FOUR_DIRECTIONS) {
		...
		cout << "Unsupported movement type! Exiting now..." << endl;
		exit(-1); 	//error! change this to throw exception
	}

	/*
	for (int i = 0; i < size; ++i) {
		//there should be another for loop for size of tuples in multi dimensional support
	}
	*/
...
/* For debugging purposes only, delete it in the final version */
void Grid::displayObstacles() const
{
