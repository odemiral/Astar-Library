...
* inlined
*/
int AStar::EuclideanHeuristic(Node *start, Node *goal) const
{
	return (int)sqrt(pow((start->getX() - goal->getX()), 2) + pow((start->getY() - goal->getY()), 2));
}
	...
	while (!open.empty()) {
		...
		for (auto next : m_grid->getNeighbors((*current))) {
			...
			if (!closed.count(*next) || cost < closed[*next].getCost()) {
				...
				int priority = cost + ManhattanHeuristic(next, goal);
